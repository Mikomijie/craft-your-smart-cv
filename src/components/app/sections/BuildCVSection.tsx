...
const BuildCVSection = () => {
  const [data, setData] = useState<CVData>(() => {
    try { const saved = localStorage.getItem("craftcv-data"); return saved ? JSON.parse(saved) : defaultCV; } catch { return defaultCV; }
  });
  ...
  const addExperience = () => setData((prev) => ({ ...prev, experience: [...prev.experience, { id: uid(), company: "", role: "", startDate: "", endDate: "", description: "" }] }));
  const updateExperience = (id: string, field: keyof Experience, value: string) => setData((prev) => ({ ...prev, experience: prev.experience.map((e) => (e.id === id ? { ...e, [field]: value } : e)) }));
  const removeExperience = (id: string) => setData((prev) => ({ ...prev, experience: prev.experience.filter((e) => e.id !== id) }));

  const addEducation = () => setData((prev) => ({ ...prev, education: [...prev.education, { id: uid(), school: "", degree: "", startDate: "", endDate: "" }] }));
  ...
  const addSkill = () => { const t = skillInput.trim(); if (t && !data.skills.includes(t)) { setData((prev) => ({ ...prev, skills: [...prev.skills, t] })); setSkillInput(""); } };
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              {data.experience.map((exp) => (
                <motion.div key={exp.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl border border-border bg-background space-y-3 relative group">
                  <button onClick={() => removeExperience(exp.id)} className="absolute top-3 right-3 p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-300 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button>
              ...
              <button onClick={addExperience} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-border text-sm text-muted-foreground hover:text-primary hover:border-primary/40 transition-all duration-300 active:scale-[0.98]">
            ...
            <div className="space-y-4">
              {data.education.map((edu) => (
                <motion.div key={edu.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-xl border border-border bg-background space-y-3 relative group">
                  <button onClick={() => removeEducation(edu.id)} className="absolute top-3 right-3 p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-300 opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button>
                  <div className="grid sm:grid-cols-2 gap-3">
              ...
              <button onClick={addEducation} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-border text-sm text-muted-foreground hover:text-primary hover:border-primary/40 transition-all duration-300 active:scale-[0.98]">
          ...
          <Section icon={Wrench} title="Skills" defaultOpen={false}>
            <div className="flex gap-2">
              ...
              <button onClick={addSkill} className="px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium transition-all duration-300 hover:brightness-110 active:scale-[0.97]">Add</button>
            </div>
